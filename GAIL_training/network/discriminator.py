...
class Discriminator(tf.Module):
    ...
    def train(self, demo_obs, demo_act, agent_obs, agent_act):
        with tf.GradientTape() as tape:
            agent_one_hot_act = tf.one_hot(agent_act, self.num_actions)  # (32, 13)
            demo_one_hot_act = tf.one_hot(demo_act, self.num_actions)  # (32, 1, 13)
            agent_input = tf.concat([agent_obs, agent_one_hot_act], axis=1)  # (32, 62)
            demo_input = tf.concat([demo_obs, demo_one_hot_act], axis=1)  # demo_obs = (32, 49)
